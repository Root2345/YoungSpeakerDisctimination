...
def cross_valid(spectrograms, datas, classes, epochs, batch_size, learning_rate):
    ...
    log_root = os.path.join('/home/s226059/workspace/git_space/workspace/logs/cnn_re_alld', datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    ...
    # for age_th in test_ageth:
        ...
        # for test_group in test_tgroup:
            ...
            if choice == True:
                ...
                train_specs = train_specs.transpose(0, 2, 1)
                valid_specs = valid_specs.transpose(0, 2, 1)
                test_specs = test_specs.transpose(0, 2, 1)

                train_specs = train_specs.reshape(train_specs.shape[0], 251, 257, 1)
                valid_specs = valid_specs.reshape(valid_specs.shape[0], 251, 257, 1)
                test_specs = test_specs.reshape(test_specs.shape[0], 251, 257, 1)
            ...
            model = cnn_model(classes, input_shape, learning_rate)
